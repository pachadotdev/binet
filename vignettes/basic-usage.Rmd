...
aggregated_planets <- galactic_federation %>% 
  group_by(planet) %>% 
  summarise(export_value = sum(export_value))

net$network_source %>% 
  graph_from_data_frame(vertices = aggregated_planets, directed = F) %>% 
  ggraph(layout = "fr") +
  geom_edge_link(aes(edge_alpha = value, edge_width = value),
                 edge_colour = "#a8a8a8") +
  geom_node_point(aes(size = export_value), color = "darkslategray4") +
...
aggregated_products <- galactic_federation %>% 
  group_by(product) %>% 
  summarise(export_value = sum(export_value))
